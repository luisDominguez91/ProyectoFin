...
public class ClientFamilyMembersReadPlatformServiceImpl implements ClientFamilyMembersReadPlatformService
{
	...
	private static final class ClientFamilyMembersMapper implements RowMapper<ClientFamilyMembersData> {
		public String schema() {
			return "fmb.id AS id, fmb.client_id AS clientId, fmb.firstname AS firstName, fmb.middlename AS middleName,"
					...
					+"fmb.ctm_motherlastname as motherlastname, fmb.ctm_curp as curp, fmb.ctm_rfc as rfc, fmb.ctm_created_by as createdBy, fmb.ctm_created_on as createdOn,"
					+"fmb.ctm_updated_by as updatedBy, fmb.ctm_updated_on as updatedOn, fmb.ctm_emailAddress as emailAddress,fmb.ctm_nss as nss,fmb.ctm_phone_no as phoneNo, fmb.ctm_is_active as active"
		...
		public ClientFamilyMembersData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			final String updatedBy=rs.getString("updatedBy");
			final LocalDate updatedOn= JdbcSupport.getLocalDate(rs, "updatedOn");
			...
			return ClientFamilyMembersData.instance(id, clientId, firstName, middleName, lastName,
					...
					motherlastname, curp, rfc, createdBy,  createdOn, updatedBy, updatedOn,
					emailAddress, nss,	phoneNo,  active);
		}
	}
	...
	public ClientFamilyMembersData retrieveTemplate() {
		...
		final List<CodeValueData> maritalStatusOptions = new ArrayList<>(
				this.codeValueReadPlatformService.retrieveCodeValuesByCode(ClientApiConstants.MARITALSTATUS));
		...
		final List<CodeValueData> relationshipOptions = new ArrayList<>(
				this.codeValueReadPlatformService.retrieveCodeValuesByCode(ClientApiConstants.RELATIONSHIP));
