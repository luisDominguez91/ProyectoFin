 ...
 * "License"); you may n|ot use this file except in compliance
...
public class LoanUtilService {
    ...
    private final static Logger logger = LoggerFactory.getLogger(LoanUtilService.class);
    ...
    public LocalDate getCalculatedRepaymentsStartingFromDate(final Loan loan) {
        logger.info("SGV getCalculatedRepaymentsStartingFromDate{");
        final CalendarInstance calendarInstance = this.calendarInstanceRepository.findCalendarInstaneByEntityId(loan.getId(),
                CalendarEntityType.LOANS.getValue());
    ...
    private LocalDate calculateRepaymentStartingFromDate(final LocalDate actualDisbursementDate, final Loan loan, final Calendar calendar, 
        ...
        logger.info("SGV calculateRepaymentStartingFromDate {");
        LocalDate calculatedRepaymentsStartingFromDate = loan.getExpectedFirstRepaymentOnDate();
        logger.info("SGV getExpectedFirstRepaymentOnDate()" + calculatedRepaymentsStartingFromDate);
        if (calendar != null) {// sync repayments
            logger.info("SGV calendar != null");
            if (calculatedRepaymentsStartingFromDate == null && !calendar.getCalendarHistory().isEmpty() &&
                    calendarHistoryDataWrapper != null) {
                logger.info("SGV calculatedRepaymentsStartingFromDate == null && !calendar.getCalendarHistory().isEmpty() && calendarHistoryDataWrapper != null");
                ...
                logger.info("SGV calculatedRepaymentsStartingFromDate" + calculatedRepaymentsStartingFromDate);
            ...
            if (calculatedRepaymentsStartingFromDate == null) {
                ...
                if (repaymentScheduleDetails != null) {// Not expecting to be
                    ...
                    if(!repaymentScheduleDetails.getRepaymentPeriodFrequencyType().isFTAndEndOfMonth()){
                        ...
                        logger.info("SGV isFTAndEndOfMonth");
                        logger.info("SGV actualDisbursementDate:" + actualDisbursementDate);
                        calculatedRepaymentsStartingFromDate= Quincena.sumarQuincenas(actualDisbursementDate,repayEvery);
                        logger.info("SGV calculatedRepaymentsStartingFromDate:" + calculatedRepaymentsStartingFromDate);
                    }
                }
            }
    ...
    private LocalDate generateCalculatedRepaymentStartDate(final CalendarHistoryDataWrapper calendarHistoryDataWrapper,
        ...
        logger.info("SGV generateCalculatedRepaymentStartDate");
        final LoanProductRelatedDetail repaymentScheduleDetails = loan.repaymentScheduleDetail();
        final WorkingDays workingDays = this.workingDaysRepository.findOne();
        ...
        if( historyList!=null && historyList.size() > 0) {
            if(repaymentScheduleDetails != null){
                ...
                if(!repaymentScheduleDetails.getRepaymentPeriodFrequencyType().isFTAndEndOfMonth()){
                    ...
                    logger.info("SGV isFTAndEndOfMonth");
                    logger.info("SGV actualDisbursementDate:" + actualDisbursementDate);
                    Quincena.sumarQuincenas(actualDisbursementDate,repayEvery);
                    logger.info("SGV calculatedRepaymentsStartingFromDate:" + calculatedRepaymentsStartingFromDate);
                }
            }
         }
