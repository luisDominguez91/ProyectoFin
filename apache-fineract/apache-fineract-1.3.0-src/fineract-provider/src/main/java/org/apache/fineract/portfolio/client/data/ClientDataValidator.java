...
public final class ClientDataValidator {
    ...
    public void validateForCreate(final String json) {
        ...
        if (this.configurationReadPlatformService
            ...
            if(!this.fromApiJsonHelper.parameterExists(ClientApiConstants.migrationIdParamName, element)) {
        ...
        // Se agregan validaciones de los nuevos campos: {
        ...
        if (this.fromApiJsonHelper.parameterExists(ClientApiConstants.rfcParamName, element)) {
            ...
            baseDataValidator.reset().parameter(ClientApiConstants.rfcParamName).value(rfc).ignoreIfNull()
                    .notExceedingLengthOf(13);
        }
        if (this.fromApiJsonHelper.parameterExists(ClientApiConstants.maritalStatusIdParamName, element)) {
            final Integer maritalStatusId = this.fromApiJsonHelper.extractIntegerSansLocaleNamed(ClientApiConstants.maritalStatusIdParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.maritalStatusIdParamName).value(maritalStatusId).integerGreaterThanZero();
        }
        ...
        if (this.fromApiJsonHelper.parameterExists(ClientApiConstants.phoneNoParamName, element)) {
            final String phoneNo = this.fromApiJsonHelper.extractStringNamed(ClientApiConstants.phoneNoParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.phoneNoParamName).value(phoneNo).ignoreIfNull()
                    .notExceedingLengthOf(20);
        }
        if (this.fromApiJsonHelper.parameterExists(ClientApiConstants.sonsNoParamName, element)) {
            final String sonsNo = this.fromApiJsonHelper.extractStringNamed(ClientApiConstants.sonsNoParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.sonsNoParamName).value(sonsNo).ignoreIfNull()
                    .notExceedingLengthOf(20);
        }
        if (this.fromApiJsonHelper.parameterExists(ClientApiConstants.filiationIdParamName, element)) {
            final Integer filiationId = this.fromApiJsonHelper.extractIntegerSansLocaleNamed(ClientApiConstants.filiationIdParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.filiationIdParamName).value(filiationId).integerGreaterThanZero();
        }
        if (this.fromApiJsonHelper.parameterExists(ClientApiConstants.nssParamName, element)) {
    ...
    private void validateIndividualNamePartsCannotBeUsedWithFullname(final JsonElement element, final DataValidatorBuilder baseDataValidator) {
        ...
        final String motherlastnameParamName = this.fromApiJsonHelper.extractStringNamed(ClientApiConstants.motherlastnameParamName, element);
        if (StringUtils.isNotBlank(motherlastnameParamName)) {
            final String fullnameParam = this.fromApiJsonHelper.extractStringNamed(ClientApiConstants.fullnameParamName, element);
            baseDataValidator.reset().parameter(ClientApiConstants.fullnameParamName).value(fullnameParam)
                    .mustBeBlankWhenParameterProvided(ClientApiConstants.motherlastnameParamName, motherlastnameParamName);
        }
    }
    ...
    private void validateRequiredIndividualNamePartsExist(final JsonElement element, final DataValidatorBuilder baseDataValidator) {
        ...
        baseDataValidator.reset().parameter(ClientApiConstants.lastnameParamName).value(lastnameParamName).notBlank()
                .notExceedingLengthOf(50);
        final String motherlastnameParamName = this.fromApiJsonHelper.extractStringNamed(ClientApiConstants.motherlastnameParamName, element);
        baseDataValidator.reset().parameter(ClientApiConstants.motherlastnameParamName).value(motherlastnameParamName).ignoreIfNull()
                .notExceedingLengthOf(50);
    }
    ...
    private boolean isIndividualNamePartParameterPassed(final JsonElement element) {
        return this.fromApiJsonHelper.parameterExists(ClientApiConstants.firstnameParamName, element)
                ...
                || this.fromApiJsonHelper.parameterExists(ClientApiConstants.motherlastnameParamName, element);
    }
    ...
    private boolean isIndividualNameProvided(final JsonElement element) {
        ...
        return StringUtils.isNotBlank(firstname) || StringUtils.isNotBlank(middlename) || StringUtils.isNotBlank(lastname)|| StringUtils.isNotBlank(motherlastname);
    }
