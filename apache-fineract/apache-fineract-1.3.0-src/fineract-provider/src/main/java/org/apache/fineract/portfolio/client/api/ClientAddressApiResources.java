...
public class ClientAddressApiResources {
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	@ApiOperation(value = "delete an address for a Client", notes = "All the address fields can be delete by using delete client address API\n" + "\n" + "Mandatory Fields\n" + "type and addressId")
	@ApiImplicitParams({@ApiImplicitParam(value = "body", required = true, paramType = "body", dataType = "body", format = "body", dataTypeClass = ClientAddressApiResourcesSwagger.DeleteClientClientIdAddressesRequest.class)})
	@ApiResponses(value = { @ApiResponse(code = 200, message = "OK", response = ClientAddressApiResourcesSwagger.DeleteClientClientIdAddressesResponse.class)})
	public String deleteClientAddress(@PathParam("clientid") @ApiParam(value = "clientId") final long clientid, @PathParam("addressid") @ApiParam(value = "addressId") final long addressid, final String apiRequestBodyAsJson) {

		final CommandWrapper commandRequest = new CommandWrapperBuilder().deleteClientAddress(clientid,addressid).withJson(apiRequestBodyAsJson).build();
	...
	@GET
	@Path("addresses/cp/{postalCode}")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrievePostalCode(@PathParam("postalCode") final String postalCode, @Context final UriInfo uriInfo) {
		this.context.authenticatedUser().validateHasReadPermission(ClientApiConstants.CLIENT_RESOURCE_NAME);
		final PostalCodeData temp= this.readPlatformService.retrievePostalCode(postalCode);
		return this.toApiJsonSerializer.serialize(temp);
	}
	@GET
	@Path("curp/{curp}")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrieveCURP(@PathParam("curp") final String curp, @Context final UriInfo uriInfo) {
		this.context.authenticatedUser().validateHasReadPermission(ClientApiConstants.CLIENT_RESOURCE_NAME);
		final CURPData temp= this.readPlatformService.retrieveCURP(curp);
		return this.toApiJsonSerializer.serialize(temp);
	}
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retriveStates(@Context final UriInfo uriInfo){
		this.context.authenticatedUser().validateHasReadPermission(ClientApiConstants.CLIENT_RESOURCE_NAME);
		final List<StateData> states = this.readPlatformService.catalogoEstados();
		return this.toApiJsonSerializer.serialize(states);
	}
